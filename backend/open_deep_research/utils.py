...
DUCKDUCKGO_SEARCH_DESCRIPTION = (
    "A free web search engine. Useful for when you need to answer questions "
    "about current events. No API key required."
)

# Global semaphore to limit concurrent DuckDuckGo searches (prevents rate limiting
# when multiple models search in parallel during comparison mode)
...
async def duckduckgo_search(
    ...
    all_results: dict[str, dict] = {}  # deduplicate by URL

    def _search_sync(query: str, max_res: int) -> list[dict]:
        with DDGS() as ddgs:
        ...
        for attempt in range(3):
            try:
                ...
                if attempt < 2:
                    ...
                    logging.warning(f"DuckDuckGo search attempt {attempt+1} failed for '{query}': {e}, retrying in {wait}s")
                    await asyncio.sleep(wait)
                else:
                    logging.warning(f"DuckDuckGo search failed for query '{query}' after 3 attempts: {e}")

    if not all_results:
        return "No search results found. Please try different search queries."

    formatted_output = "Search results:\n\n"
    for i, (url, result) in enumerate(all_results.items()):
