...
async def clarify_with_user(state: AgentState, config: RunnableConfig) -> Command[Literal["write_research_brief", "__end__"]]:
    ...
    # Try structured output, fall back to skipping clarification for incompatible models
    try:
        clarification_model = (
            configurable_model
        ...
        # Fallback: if structured output fails, skip clarification and proceed
        import logging
        logging.getLogger(__name__).warning(
            f"Clarification structured output failed, skipping: {e}"
        )
        return Command(goto="write_research_brief")
...
async def write_research_brief(state: AgentState, config: RunnableConfig) -> Command[Literal["research_supervisor"]]:
    ...
    # Try structured output first, fall back to raw text for models that don't support it well
    research_brief = None
    try:
        research_model = (
            configurable_model
        ...
        # Fallback: some models (e.g. GLM-4.7 via Volcano Engine) don't support structured output
        # Invoke model directly and use raw text as the research brief
        import logging
        logging.getLogger(__name__).warning(
            f"Structured output failed, falling back to raw text: {e}"
        )
        raw_model = configurable_model.with_config(research_model_config)
        raw_response = await raw_model.ainvoke([HumanMessage(content=prompt_content)])
        research_brief = raw_response.content if hasattr(raw_response, 'content') else str(raw_response)
